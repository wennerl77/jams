...
if [ "$START_SYSTEMS" = true ]; then
    ...
    if vagrant status boca 2>/dev/null | grep -q "running"; then
        echo "  -> Starting Docker Compose in BOCA VM..."
        vagrant ssh boca -c "if [ -d /var/www/boca ]; then cd /var/www/boca && docker compose up -d; else docker compose up -d 2>/dev/null || true; fi"
    fi
    if vagrant status helium 2>/dev/null | grep -q "running"; then
        echo "  -> Starting Docker Compose in Helium VM..."
        vagrant ssh helium -c "if [ -d /var/www/helium ]; then cd /var/www/helium && docker compose up -d; else docker compose up -d 2>/dev/null || true; fi"
    fi
else
...
if command -v vagrant &> /dev/null && vagrant status boca 2>/dev/null | grep -q "running"; then
    vagrant ssh boca -c "docker exec -i boca-db psql -U boca boca" < "$SCRIPT_DIR/boca/01_seed_benchmark.sql"
...
if command -v vagrant &> /dev/null && vagrant status helium 2>/dev/null | grep -q "running"; then
    vagrant ssh helium -c "docker exec -i helium-db mysql -u helium -psecret helium" < "$SCRIPT_DIR/helium/01_seed_benchmark.sql"
